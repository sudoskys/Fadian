...
class Command:
    ...
    def _extract_docs(self, subcmd_name: str, text: str, markdown: bool = False) -> str:
        ...
        if markdown:
            result = [
                "
{self._get_cmd_with_param(subcmd_name, markdown) or code(cmd_name)} · {re.search(r'(.+)', self[subcmd_name].__doc__ or '')[1].strip()}
",
            ]
            return "\n".join(result)
    ...
    async def help(self, command: Optional[str], search_str: Optional[str] = None):
        ...
        :param opt search_str: 搜索的文字，只有 commandsearch 时有效
        :alias: h
        """
    ...
    async def timeout(self, seconds: Optional[int], _type: Optional[str]):
        ...
        使用 ,{cmd_name} wait off 可关闭验证时间限制
    ...
    async def flood(self, limit: Optional[int]):
        ...
        并对记录收集机器人发送轰炸的用户数量、轰炸开始时间、轰炸结束时间、轰炸时长
        （由于不存在隐私问题，此操作为强制性）

        请参阅 ,{cmd_name} h flood_username 了解更多有关创建临时频道的机制
        请参阅 ,{cmd_name} h flood_act 查看轰炸结束后的处理方式
    ...
    async def custom_rule(self, *rule: Optional[str]):
        ...
        使用 ,{cmd_name} custom_rule -c 可删除规则
